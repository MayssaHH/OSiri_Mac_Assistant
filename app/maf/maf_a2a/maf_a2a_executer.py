"""
MAF A2A Executor
Handles incoming A2A requests and executes Slack/Outlook tasks using the planner+executor pattern.
"""
import json
import logging
import os
...
from maf_mcp.maf_agent import build_planner_agent, build_executor_agent
from agent_framework.openai import OpenAIChatClient
from dotenv import load_dotenv

load_dotenv()

logger = logging.getLogger("maf-a2a-executor")
logger.setLevel(logging.INFO)


class MAFAgentExecutor(AgentExecutor):
    """
    A2A-facing executor for MAF tasks.
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue) -> None:
        await event_queue.enqueue_event(
            new_agent_text_message("cancel not supported for MAFAgentExecutor")
        )
